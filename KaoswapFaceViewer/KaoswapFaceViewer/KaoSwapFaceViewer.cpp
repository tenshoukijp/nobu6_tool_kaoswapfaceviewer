...
#include "KaoSwapFaceViewerForm.h"
...
void MallocBushouFaceDataBuffer() {
	bushou_face_data = new BMP_INDEX_COLOR8_FACE_DATA[KaoswapFaceViewer::BUSHOU_NUM];
}
...
bool ImportKaoDataFileToProgramStruct(String^ strInputFileName) {
	...
	const int iBushouNum = KaoswapFaceViewer::BUSHOU_NUM;
	// 先頭のbushou_num分はスキップ
	fin.seekg(iBushouNum);
...
Image^ GetBushouFaceImage(int iBushouID) {
	if (0 <= iBushouID && iBushouID < KaoswapFaceViewer::BUSHOU_NUM) {
		BMP_INDEX_COLOR8_FACE_FILE_FORMAT face;
		face.header = binBitMapHeaderNobu6;
...
int main(cli::array<System::String ^> ^args)
{
	...
	String^ strTargetFile = "kaoswap.n6p";
	if (args->Length > 0) {
	...
	KaoswapFaceViewer::BUSHOU_NUM = iBushouNum;

	MallocBushouFaceDataBuffer();
	// メイン ウィンドウを作成して、実行します
	Application::Run(gcnew KaoswapFaceViewer(strTargetFile));
